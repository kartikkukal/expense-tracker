...
class AddPeriodicalIncome:
    ...
    def run(self):
        ...
        ttk.Button(button_frame, text="Create", width=10, style="Accent.TButton", 
                   command=self.add_periodical_dialog).pack(side=tk.RIGHT, padx=(0, 10))
    ...
    def add_periodical_dialog(self):
        ...
        try:
            ...
            # Add income periodical
            
            def add_periodical_execute():

                self.root.mysql.add_periodical((note, wallet, None, frequency, from_date_time, till_date_time, amount, False))
            ...
            if till_date_time < datetime.datetime.now(datetime.timezone.utc):
                Confirmation(self.root, "Since the till date is before current date, all the income entries will be added at once without creating an entry for perioidcal.", add_periodical_execute).show()
                return
