...
class AddPeriodicalExpense:
    ...
    def run(self):
        ...
        self.dialog.title("Add Periodical")
        ...
        # Date entry
        ttk.Label(frame, text="From:").grid(row=2, column=0, pady=(0, 10))

        # Date frame
        date_frame = ttk.Frame(frame)
        date_frame.grid(row=2, column=1, sticky="EW", pady=(0, 10))

        from_date = datetime.datetime.now(zoneinfo.ZoneInfo("Asia/Kolkata")).date()
        
        # Year entry
        self.from_year = tk.StringVar(date_frame, value=from_date.year)
        ttk.Entry(date_frame, width=4, textvariable=self.from_year).grid(row=0, column=1, sticky="EW", padx=(0, 10))

        # Month entry
        self.from_month = tk.StringVar(date_frame, value=from_date.month)
        ttk.Entry(date_frame, width=2, textvariable=self.from_month).grid(row=0, column=2, sticky="EW", padx=(0, 10))

        # Day entry
        self.from_day = tk.StringVar(date_frame, value=from_date.day)
        ttk.Entry(date_frame, width=2, textvariable=self.from_day).grid(row=0, column=3, sticky="EW", padx=(0, 15))

        # Frequency entry
        ttk.Label(frame, text="Frequency:").grid(row=2, column=2, pady=(0, 10))

        self.frequency_options = ("Daily", "Weekly", "Monthly", "Yearly")
        self.frequency_selected = tk.StringVar(value=self.frequency_options[2])
        
        ttk.OptionMenu(frame, self.frequency_selected, None, *self.frequency_options).grid(row=2, column=3, columnspan=2, sticky="EW", pady=(0, 10))

        # Date entry
        ttk.Label(frame, text="Till:").grid(row=3, column=0)

        # Date frame
        date_frame = ttk.Frame(frame)
        date_frame.grid(row=3, column=1, sticky="EW")

        till_date = datetime.datetime.now(zoneinfo.ZoneInfo("Asia/Kolkata")).date()
        ...
        # Amount entry
        ttk.Label(frame, text="Amount:").grid(row=3, column=2)

        self.amount = tk.StringVar(frame)
        ttk.Entry(frame, width=6, textvariable=self.amount).grid(row=3, column=3, columnspan=2, sticky="EW")
        ...
        ttk.Button(button_frame, text="Create", width=10, style="Accent.TButton", 
                   command=self.add_periodical).pack(side=tk.RIGHT, padx=(0, 10))
    ...
    def add_periodical(self):
        ...
        try:
            ...
            # Add expenses periodical

            def add_periodical_execute():

                self.root.mysql.add_periodical((note, wallet, category, frequency, from_date_time, till_date_time, amount, True))
            ...
            if till_date_time < datetime.datetime.now(datetime.timezone.utc):
                Confirmation(self.root, "Since the till date is before current date, all the expenses entries will be added at once without creating an entry for perioidcal.", add_periodical_execute).show()
                return
            
            add_periodical_execute()
