...
class ViewPeriodicalExpense:
    ...
    def run(self, id):
        ...
        self.category = tk.StringVar(value=self.root.mysql.get_category_by_id(self.record[3])[0])
        ...
        ttk.Button(button_frame, text="Update", width=10, style="Accent.TButton", command=self.update_periodical).pack(side=tk.RIGHT, padx=(0, 10))
    ...
    def update_periodical(self):
        ...
        try:
            ...
            def update_periodical_execute():
                ...
                self.root.mysql.update_periodical_by_id(self.record[0], (note, wallet, category, frequency, from_date_time, till_date_time, amount, True))
            ...
            if till_date_time < datetime.datetime.now(datetime.timezone.utc):
                self.root.debug_message("UpdatePeriodicalExpense", "Till date time is less than current date.")
                Confirmation(self.root, "Since the till date is before current date, all the expenses entries will be added at once without creating an entry for perioidcal.", update_periodical_execute).show()
                return
